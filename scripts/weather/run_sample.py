...
Runs the pipeline in dependency order with caching everywhere:
    ...
    5. parse_nearby_asos nearby ASOS METAR normalization (stdlib, cached)
    6. build_intraday_state  point-in-time derived state, offline
    7. kalshi_meta       Kalshi events/markets metadata for the 4 series
    8. kalshi_trades_candles   bounded candle + public trade snapshots
    9. load_sqlite       recreate the portable SQLite query layer, offline
   10. validate          schema checks, cross-source agreement,
...
def main() -> int:
    ...
    for name, cmd in steps:
        resolved = [str(SCRIPTS / c) if c.endswith(".py") else c
                    for c in cmd]
