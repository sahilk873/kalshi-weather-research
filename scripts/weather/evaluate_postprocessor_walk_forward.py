"""Evaluate PIT-safe bias, quantile, or EMOS postprocessors by walk-forward fold."""
...
from postprocess_station_bias import fit_bias, apply_bias
from quantile_postprocess import fit_quantiles, apply_quantiles
from emos import fit_emos, apply_emos
from boosted_postprocess import fit_boosted, apply_boosted


def _apply(method: str, train: list[dict], test: list[dict], labels: list[dict]) -> list[dict]:
    from common import parse_utc_iso
    as_of = min(parse_utc_iso(row["decision_time_utc"]) for row in test)
    if method == "bias":
        return apply_bias(test, fit_bias(train, labels, as_of=as_of))
    if method == "quantile":
        return apply_quantiles(test, fit_quantiles(train, labels, as_of=as_of))
    if method == "emos":
        return apply_emos(test, fit_emos(train, labels, as_of=as_of))
    if method == "boosted":
        return apply_boosted(test, fit_boosted(train, labels, as_of=as_of))
    raise ValueError(f"unknown method: {method}")


def evaluate(forecasts: list[dict], labels: list[dict], method: str,
             train_days: int = 365, test_days: int = 30,
             embargo_days: int = 0) -> tuple[list[dict], list]:
    ...
    for fold in folds:
        ...
        train = [row for row in forecasts if row.get("outcome_local_date") in train_dates]
        test = [row for row in forecasts if row.get("outcome_local_date") in test_dates]
        corrected = _apply(method, train, test, labels)
        good, bad = build_rows(corrected, labels)
        rejected.extend(bad)
...
def main() -> None:
    ...
    parser.add_argument("--method", choices=("bias", "quantile", "emos", "boosted"), required=True)
    ...
    rows, rejected = evaluate(read_csv(args.forecasts), read_csv(args.labels), args.method, args.train_days, args.test_days, args.embargo_days)
