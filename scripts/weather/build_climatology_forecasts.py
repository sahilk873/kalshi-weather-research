"""Build a point-in-time monthly climatology forecast schedule.

This is a deliberately simple P1 hurdle-rate model.  For each requested
forecast target it uses only labels whose publication/availability timestamp
is no later than the decision time, excludes the target date, and estimates a
normal distribution from prior same-city, same-month observations.  It never
claims to be an NWP forecast or a settlement-rule resolver.
"""
...
def build_rows(schedules: list[dict], labels: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    for row in schedules:
        ...
        if not city or temp_type not in {"high", "low"} or decision is None or target is None:
            ...
            continue
        training = [value for day, available, value in index[(city, temp_type)]
                    if day != target and day.month == target.month and day < target
                    and available <= decision]
        if not training:
            rejected.append({"event_ticker": row.get("event_ticker", ""), "reason": "no_prior_available_labels"})
            continue
        mean = sum(training) / len(training)
        sigma = math.sqrt(sum((value - mean) ** 2 for value in training) / len(training)) if len(training) > 1 else None
        ...
        output.append({
            ...
            "model_name": "climatology", "model_version": "monthly_v1", "city": city,
...
def render(schedule_path: Path, labels_path: Path, output_path: Path, rejection_path: Path | None = None) -> tuple[int, int]:
...
def main() -> None:
    ...
    print(render(args.schedule, args.labels, args.output, args.rejections))
